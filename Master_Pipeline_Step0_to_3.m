%% Master_Pipeline_Step0_to_3.m
%  TTC Round 9 - Reifenmodell-Pipeline (RP25e, Hoosier 43075)
%  Schritte 0-3: Run-Lookup, Laden/Filtern/Klassifizieren, Fit der
%                Stuetzreifen, Fy-Pure-Fit des Zielreifens
%
% Autor: Lambo || Datum: 22.06.26
%
%  Aufbau:
%    Abschnitt 1: KONFIGURATION  <- hier Pfade/Parameter anpassen
%    Abschnitt 2: AUSFUEHRUNG    <- ruft die vier Schritte nacheinander auf
%    %% Functions: lokale Funktionen (Step0/1/2/3 + Hilfsfunktionen)
%
...
TARGET_DIR    = '3_Target_Model';        % Output von Step3
...
DOWNSAMPLE_FACTOR = 3;   % fuer Step2/3 Fit (Bessel hat bereits geglaettet)
...
fprintf('\n╔══════════════════════════════════════════════════════════╗\n');
fprintf('║  STEP 3: Fy-Pure-Fit des Zielreifens (%s)               ║\n', TARGET_TIRE_CODE);
fprintf('╚══════════════════════════════════════════════════════════╝\n');
target_model = local_Step3_Fit_Target_Tire(SEG_DIR, ...
    'TireCode', TARGET_TIRE_CODE, ...
    'OutDir', TARGET_DIR, ...
    'DownsampleFactor', DOWNSAMPLE_FACTOR); %#ok<NASGU>

fprintf('\nFERTIG. Naechster Schritt: Step4 (mu-Verhaeltnis-Schaetzung fuer Fx\n');
fprintf('auf Basis der Stuetzreifen) und Step5 (Modellzusammenfuehrung + Export).\n');
...
% ═══════════════════════════════════════════════════════════════════════
% STEP 3: Fy-Pure-Fit des Zielreifens (43075, keine Fx/Combined-Daten)
% ═══════════════════════════════════════════════════════════════════════
function target_model = local_Step3_Fit_Target_Tire(seg_dir, varargin)
% Fittet das MF6.x Fy-Pure-Modell fuer den Zielreifen 43075 (RP25e).
% KEINE Fx/Combined-Daten verfuegbar (16" OD 10"-Reifen, Offset-Adapter
% verhindert Drive/Brake-Messung, siehe RunGuide).
%
% Liefert NUR das Fy-Pure-Teilmodell. Die Fx-Pure- und Combined-Parameter
% werden in Step4 ueber die mu-Verhaeltnis-Schaetzung aus den Stuetzreifen
% (43100, D0571, MRF_ZTD1, siehe Step2) ergaenzt.

p = inputParser;
addParameter(p, 'TireCode', '43075', @ischar);
addParameter(p, 'OutDir', '3_Target_Model', @ischar);
addParameter(p, 'Geometry', local_default_target_geometry(), @isstruct);
addParameter(p, 'DownsampleFactor', 3, @isnumeric);
parse(p, varargin{:});
code      = p.Results.TireCode;
OUT_DIR   = p.Results.OutDir;
geo       = p.Results.Geometry;
DS_FACTOR = p.Results.DownsampleFactor;

INFLPRES = 82740;   % [Pa] 12 psi - identisch zu Step2
NOMPRES  = 82740;
FNOMIN   = 700;     % [N]  gleiche Konvention wie Stuetzreifen

if ~isfolder(OUT_DIR), mkdir(OUT_DIR); end

fprintf('════════════════════════════════════════════════════════════\n');
fprintf('Fitte Zielreifen: %s  (NUR Fy Pure - keine Fx/Combined-Daten)\n', code);

seg_file = fullfile(seg_dir, sprintf('Step1_Segments_%s.mat', code));
if ~isfile(seg_file)
    error('Step3: Segmentdatei fehlt fuer %s (%s). Erst Step1 ausfuehren.', code, seg_file);
end
S = load(seg_file);
segs = S.tire_segs;

if isempty(segs)
    error('Step3: Keine Segmente fuer %s - Fit nicht moeglich.', code);
end
...
if isempty(td_lat)
    error('Step3: Keine Lateral-Segmente fuer %s - Fy-Pure-Fit nicht moeglich.', code);
end

% Defensiver Hinweis: 43075 hat laut RunGuide keine Drive/Brake-Daten.
% Falls hier doch Longitudinal/Combined-Segmente auftauchen (z.B. durch
% Fehlklassifikation kleiner SL-Drifts waehrend Lateral-Sweeps), werden
% sie bewusst NICHT gefittet - Step4 liefert die Fx-Schaetzung separat.
if ~isempty(td_long) || ~isempty(td_comb)
    warning(['Step3 [%s]: %d Longitudinal- und %d Combined-Segmente gefunden, ' ...
             'obwohl der Zielreifen laut RunGuide keine Drive/Brake-Daten hat. ' ...
             'Vermutlich Fehlklassifikation kleiner SL-Drifts waehrend des ' ...
             'Lateral-Sweeps - werden NICHT gefittet.'], code, numel(td_long), numel(td_comb));
end

td_lat = local_downsample_segs(td_lat, DS_FACTOR);
fprintf('  Punkte nach DS (Faktor %d): Lateral=%d\n', DS_FACTOR, local_count_pts(td_lat));
...
tm.UNLOADED_RADIUS = geo.UNLOADED_RADIUS;
tm.WIDTH           = geo.WIDTH;
tm.ASPECT_RATIO    = geo.ASPECT_RATIO;
tm.RIM_RADIUS      = geo.RIM_RADIUS;
tm.RIM_WIDTH       = geo.RIM_WIDTH;

[tm, ~] = fit(tm, td_lat, "Limits", "Parameters", ["FZMAX","ALPMIN","ALPMAX"]);
tm.FZMIN = 0;
...
fit_meta = struct('FittedOn', datestr(now), 'INFLPRES', INFLPRES, ...
    'FNOMIN', FNOMIN, 'Fy_Pure_rmse', NaN, ...
    'Fx_Pure_rmse', NaN, 'Fy_Comb_rmse', NaN, 'Fx_Comb_rmse', NaN, ...
    'n_lateral', numel(td_lat), 'n_longitudinal', 0, 'n_combined', 0, ...
    'has_own_fx_data', false, ...
    'note', 'Fx/Combined-Parameter fehlen - siehe Step4 fuer Schaetzung.');
...
if tm.PDY2 > 0
    warning('Step3 [%s]: PDY2=%.4f positiv (keine Degressivitaet erkennbar).', code, tm.PDY2);
end

fprintf('  Fy-Pure-Parameter:\n');
fprintf('    PDY1=%.4f  PDY2=%.4f  PCY1=%.4f  PKY1=%.4f\n', ...
    tm.PDY1, tm.PDY2, tm.PCY1, tm.PKY1);

target_model.tire_code = code;
target_model.tm        = tm;
target_model.fit_meta   = fit_meta;

out_file = fullfile(OUT_DIR, sprintf('Step3_TargetModel_%s.mat', code));
save(out_file, 'tm', 'fit_meta', 'code');
fprintf('\n  Gespeichert: %s\n', out_file);
fprintf('  HINWEIS: Modell enthaelt noch KEINE Fx/Combined-Parameter.\n');
fprintf('  Naechster Schritt: Step4 (mu-Verhaeltnis-Schaetzung aus Stuetzreifen).\n');
fprintf('════════════════════════════════════════════════════════════\n');
end


function geo = local_default_target_geometry()
% Geometrie fuer 43075 (16.0x7.5-10 R20), abgeleitet aus der
% Nominal-Reifenbezeichnung selbst (16.0" Aussendurchmesser, 7.5"
% Sektionsbreite, 10" Felgendurchmesser). Das ist die zuverlaessigste
% Quelle ohne spekulative Katalogwerte - keine belastbare externe
% Quelle mit exakten Messwerten (Diameter/Width unter Last) gefunden.
% Bei Bedarf mit '2356 Summary Tables.xlsx' (Tire Weights) verifizieren.

IN2M = 0.0254;

geo.UNLOADED_RADIUS = (16.0/2) * IN2M;   % 0.2032 m
geo.WIDTH           = 7.5 * IN2M;        % 0.1905 m
geo.ASPECT_RATIO    = 0.45;              % Schaetzwert, wie Stuetzreifen
geo.RIM_RADIUS      = (10.0/2) * IN2M;   % 0.1270 m
geo.RIM_WIDTH       = 7.0 * IN2M;        % 0.1778 m (RP25e: 7" Felge)
end